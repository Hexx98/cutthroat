...
- **Combo points are secret.** Addons may display them but not read, compare or do math on them. Each pip is a StatusBar ranged `[i-1, i]` that is handed the raw count; the engine clamps it, lighting pips 1..cp without the addon ever seeing the number.
